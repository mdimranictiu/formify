...
const PropertiesPanel = () => {
  const {handleApplyFieldsetName,deleteFieldFromFieldset,handleApplyEdit,PropertiesPanelShow,handleDeleteFieldset, fieldsets,selectedFieldsetId,setFieldsets, setSelectedFieldsetId } = useContext(FormBuilderContext);
...
/>
            ...
            <div className='flex flex-col gap-3 py-5'>
              ...
              {fields?.map((field,index)=>(
                ...
                <span><FaEdit onClick={()=>handleEditField(selectedFieldsetId,field.id)} className='text-xl'></FaEdit></span>
            ...
            <div className='p-2 flex justify-between'>
              ...
              <button onClick={()=>handleApply(fieldsetName)} className='btn btn-soft btn-info px-10'>Apply</button>
            </div>
            </div>
  ...
  <div className="modal-box w-11/12 max-w-2xl">
        ...
        {(edit?.options || []).map((option, index) => (
          <div key={index} className="flex gap-2 mb-2 items-center">
            ...
            <button
              type="button"
              onClick={() => {
                const filteredOptions = edit.options.filter((_, i) => i !== index);
    ...
    <div className="modal-action">
      ...
      <button className="btn btn-success" onClick={() => handleApplyEditSection(edit)}>Apply</button>
    </div>
  </div>
</dialog>
